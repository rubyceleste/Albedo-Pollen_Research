#' Home / CRAN / ClimClass / R/thornthwaite.R
#' 
#' 
#' R/thornthwaite.R
                        ...
                        snow_melt_coeff = 1) 
{
  ...
  if(((!Tn.exists | !Tx.exists) & !Tm.exists) | !P.exists)
    print("Both T and P series required!", quote=FALSE) else {
      ...
      for (ii in 1:length(years)) {
        ...
        if(Tn.exists & Tx.exists & !Tm.exists)
          ...
          if (sum(is.na(zzz$Prec1)) != 0 | sum(is.na(zzz$Tmean1)) != 0) 
            ...
            tSnowAcc <- data.frame(tSnowAcc, SnowAcc)
          }
      }
      ...
      names(tSnowAcc) <- c("Month", years)
      row.names(tSnowAcc) <- month_names
      list_thornt <- list(round(tPrec[-1], 1), 
                          round(tET0[-1], 1), 
                          round(tStor[-1], 1), 
                          round(tPmE[-1], 1), 
                          round(tDef[-1], 1), 
      ...
      names(list_thornt) <- c("Precipitation", "Et0", "Storage", 
                              "Prec. - Evap.", "Deficit", "Surplus", "Snowpack")
      ...
      qSnowAcc <- as.data.frame(apply(ttSnowAcc[-1, ], FUN = quantile, 
                                  probs = quant, na.rm = T, MARGIN = 2))
      names(qSnowAcc) <- month_names
      
      list_quant <- list(round(qPrec, 1), 
                         round(qET0, 1), 
                         round(qStor, 1), 
                         round(qPmE, 1), 
                         round(qDef, 1), 
